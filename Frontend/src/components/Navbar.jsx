...
const Navbar = () => {
  ...
  return (
    <nav className={`w-full px-3 sm:px-6 py-3 sm:py-4 sticky top-0 z-40 transition-all duration-300 ${scrolled ? 'backdrop-blur-xl bg-black/50' : 'backdrop-blur-md bg-black/40'}`}>
      <div className="max-w-7xl mx-auto glass-navbar rounded-full px-4 sm:px-8 py-2.5 sm:py-4 flex items-center justify-between shadow-lg border border-white/10">
        {/* Logo and Brand */}
        <Link to="/" className="flex items-center gap-2 sm:gap-3">
          <img src={logocar} alt="KARZONE Logo" className="h-8 sm:h-10 w-auto" />
          <span className="hidden sm:inline text-xl sm:text-2xl font-bold text-white tracking-tight">KARZONE</span>
        </Link>
        ...
        <div className="hidden lg:flex items-center gap-1">
          ...
          <Link
            to="/cars"
            className={`px-4 py-2 text-base font-medium transition-colors ${isActive('/cars')
              ? 'text-orange-500'
              : 'text-white/80 hover:text-white'
              }`}
          >
          ...
          <Link
            to="/contact"
            className={`px-4 py-2 text-base font-medium transition-colors ${isActive('/contact')
              ? 'text-orange-500'
              : 'text-white/80 hover:text-white'
              }`}
          >
          ...
          {isLoggedIn && (
            <>
              ...
              <Link
                to="/bookings"
                className={`px-4 py-2 text-base font-medium transition-colors ${isActive('/bookings')
                  ? 'text-orange-500'
                  : 'text-white/80 hover:text-white'
                  }`}
              >
      ...
      >
        ...
        >
          <Link
            ...
            className={`block w-full px-3 py-2 rounded-lg text-base font-medium ${isActive('/') ? 'bg-gray-800 text-orange-500' : 'text-gray-200 hover:bg-gray-800'
              }`}
          >
            Home
          </Link>
          <Link
            to="/cars"
            onClick={() => setIsOpen(false)}
            className={`block w-full px-3 py-2 rounded-lg text-base font-medium ${isActive('/cars') ? 'bg-gray-800 text-orange-500' : 'text-gray-200 hover:bg-gray-800'
              }`}
          >
            Cars
          </Link>
          <Link
            to="/contact"
            onClick={() => setIsOpen(false)}
            className={`block w-full px-3 py-2 rounded-lg text-base font-medium ${isActive('/contact')
          ...
          {isLoggedIn && (
            <Link
              to="/bookings"
              onClick={() => setIsOpen(false)}
              className={`block w-full px-3 py-2 rounded-lg text-base font-medium ${isActive('/bookings')
                ? 'bg-gray-800 text-orange-500'
                : 'text-gray-200 hover:bg-gray-800'
                }`}
            >
